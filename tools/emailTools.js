...
// extracts clean email address from "Name <email@domain.com>" format
    ...
    try {
        ...
        const emails = await Promise.all(
            messages.map(async (msg) => {
                ...
                // parse clean email address for replying
                const fromEmail = parseEmail(fromHeader)
 
                return {
                    id: msg.id,
                    from: fromHeader,       // full display name + email (for classification)
                    fromEmail,              // clean email only (for sending replies)
                    subject,
                    date,
